/* ============================================================================
   СтройФінанс — Google Apps Script backend (переписано з нуля)
...
var EXPORT_TAB_ORDER = Object.freeze(['Стройки','Витрати','Накладні','Договори','Акти']);
...
function reportClientError(payload, userCode) {
  try {
    ...
    console.log('[СтройФінанс][CLIENT][ERROR] ' + e);
  }
  return true;
}
...
function logAndNotify_(user, action, details) {
  ...
  notify_(user, action, details);
}

function notify_(user, action, details) {
  ...
  var emoji = action === 'added' ? '🟢' : (action === 'updated' ? '🟡' : '🔴');
  var word = action === 'added' ? 'додав' : (action === 'updated' ? 'оновив' : 'видалила(в)');
  var message = emoji + ' <b>' + escTg_(user.name) + '</b> ' + word + ' ' + details;
  var url = 'https://api.telegram.org/bot' + token + '/sendMessage';

  var requests = chatIds.map(function (cid) {
    return {
      url: url,
      method: 'post',
  ...
  try {
    ...
    log_('INFO', 'Telegram sent', { chats: chatIds.length, codes: codes });
  } catch (e) {
    log_('WARN', 'Telegram failed', { error: String(e) });
  }
}
...
/*
  Формули нижче навмисно залишені 1:1 зі старим кодом.
...
function calculateResults_(state) {
  ...
  (state.overheads || []).forEach(function (e) {
    if (e.group_type === 'cashless') overheadCashless += e.amount;
    else if (e.group_type === 'cash') overheadCash += e.amount;
  });
  ...
  (state.projects || []).forEach(function (p) {
    ...
    (p.entries || []).forEach(function (en) {
      if (Object.prototype.hasOwnProperty.call(sums, en.section)) sums[en.section] += num_(en.amount);
    });
...
function exportExcel(userCode) {
  return withRequestContext_('exportExcel', function () {
    ...
    var settings = readSettings_();
    var exp = null;
    if (settings.export_sheet_id) {
      try { exp = SpreadsheetApp.openById(String(settings.export_sheet_id)); }
      catch (e) { log_('WARN', 'Cannot open existing export spreadsheet', { error: String(e) }); }
    }

    if (!exp) {
      exp = SpreadsheetApp.create(APP.NAME + ' — EXEL експорт');
      setSetting_('export_sheet_id', exp.getId());
    }

    try {
      DriveApp.getFileById(exp.getId()).setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
    } catch (e) {
      log_('WARN', 'Cannot change export sharing', { error: String(e) });
    }

    var first = exp.getSheets()[0];
    first.clear();
    first.setName(EXPORT_TAB_ORDER[0]);
    copyTabInto_(EXPORT_TAB_ORDER[0], first);

    for (var i = 1; i < EXPORT_TAB_ORDER.length; i++) {
      var name = EXPORT_TAB_ORDER[i];
      var sh = exp.getSheetByName(name) || exp.insertSheet(name);
      sh.clear();
      copyTabInto_(name, sh);
    }

    var resultSheet = exp.getSheetByName('Результат') || exp.insertSheet('Результат');
    resultSheet.clear();
    buildResultsSheet_(resultSheet);
    exp.setActiveSheet(first);

    log_('INFO', 'Excel export ready', { user: user.name, exportId: exp.getId() });
    return 'https://docs.google.com/spreadsheets/d/' + exp.getId() + '/export?format=xlsx';
  });
}

function copyTabInto_(name, dst) {
  var src = getSpreadsheet_().getSheetByName(name);
...
function buildResultsSheet_(dst) {
  var state = getState_();
  var rows = state.results.rows;
  var totals = state.results.totals;
  var head = ['№', 'Найменування', 'Валовий прибуток', 'Накладні', 'Безготівкові', 'Конвертація', 'Готівкові', 'Чистий прибуток', '%'];
  var data = [head];

  rows.forEach(function (r, i) {
    data.push([i + 1, r.name, r.turnover, r.allocated_overhead, r.cashless, r.conversion, r.cash_expenses, r.net_profit, r.profit_percent]);
  });
  data.push(['', 'ВСЬОГО', totals.turnover, totals.allocated_overhead, totals.cashless, totals.conversion, totals.cash_expenses, totals.net_profit, totals.profit_percent]);

  dst.getRange(1, 1, data.length, head.length).setValues(data);
  dst.setFrozenRows(1);
}
