...
function flushTelegramDigest() {
  return withRequestContext_('flushTelegramDigest', function () {
    return withWriteLock_('telegramDigest', function () {
      ...
      return { ok: false, sent: false, retry: true };
    });
  });
}
...
function sendTelegramDigest_(pending) {
  ...
  try {
    ...
    log_(ok ? 'INFO' : 'WARN', 'Telegram digest response', { chats: chatIds.length, chunks: chunks.length, codes: codes });
    return { ok: ok, configured: true };
  } catch (e) {
    log_('WARN', 'Telegram digest failed', { error: String(e) });
...
function testTelegramNow() {
  return withRequestContext_('testTelegramNow', function () {
    ...
    if (!result.ok) throw new Error('Telegram не налаштований або API повернув помилку. Перевірте telegram_bot_token та chat_ids у «Налаштування».');
    return { ok: true };
  });
}
